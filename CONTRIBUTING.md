...
5. Release Please maintains the stable release PR on `main`. Before it is merged, automation folds the matching beta entries into both the stable section of `CHANGELOG.md` and the release PR body, omitting release/checkpoint housekeeping. Merging it publishes those notes with `vX.Y.Z`, `stable`, and `latest` only after all release jobs succeed.
