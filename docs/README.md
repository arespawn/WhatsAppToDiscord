...
- **`QR refs attempts ended (408)` during pairing** – WhatsApp closed the current QR pairing window before it completed. If Baileys has already written partial unregistered auth data, WA2DC clears only the WhatsApp auth state and starts a fresh pairing prompt so it does not loop between stale login data and new QR codes. Scan the next fresh QR code or request a new `/pairwithcode` code.
