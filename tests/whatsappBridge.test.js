...
test("Discord spoiler image attachments are sent to WhatsApp as view once media", async () => {
	const harness = await setupWhatsAppHarness({ oneWay: 0b11 });
	try {
		const sharpMod = await import("sharp");
		const sharp = sharpMod?.default || sharpMod;
		const pngBytes = await sharp({
			create: {
				...
				height: 3,
				channels: 4,
				background: { r: 80, g: 20, b: 140, alpha: 1 },
			},
		})
			.png()
			.toBuffer();
		...
		assert.equal(sent, true);
		const sentContent = harness.fakeClient.sendCalls[0]?.content || {};
		assert.equal(sentContent.viewOnce, true);
		assert.equal(sentContent.document, undefined);
	} finally {
		harness.cleanup();
	}
